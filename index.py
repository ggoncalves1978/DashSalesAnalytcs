...
# =========  Tratando os dados  =========== # (Informações geradas no Jupter Lab)

df = pd.read_csv('dataset_asimov.csv')
df_cru = df.copy() # Criado para fazer algumas manipulações de strings
...
# =========  Opções para os filtros (escolha o mês / equipe)  =========== #
...
options_team = [{'label': 'Todas Equipe', 'value': 0}]
for i in df['Equipe'].unique():
    options_team.append({'label': i, 'value': i})

# =========  Função para escolha do mês =========== #
